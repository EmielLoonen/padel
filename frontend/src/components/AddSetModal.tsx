...
export default function AddSetModal({
  ...
  const fetchPlayerStats = async () => {
    try {
      ...
      const enrichedPlayers = players.map((player) => {
        const stats = leaderboard.find((p: any) => p.userId === player.id);
        return {
          ...player,
      ...
      setPlayersWithStats(enrichedPlayers);
    } catch (error) {
      console.error('Failed to fetch player stats:', error);
      // Set players without stats
      setPlayersWithStats(players.map((p) => ({ ...p, setWinRate: 0, totalSets: 0 })));
    } finally {
  ...
  const handleSmartBalance = () => {
    if (playersWithStats.length !== 4) {
      setError('Smart balance only works with 4 players');
      return;
    }

    // Sort players by set win rate (highest to lowest)
    const sortedPlayers = [...playersWithStats].sort((a, b) => {
      // Players with no sets go to the bottom
      if (a.totalSets === 0 && b.totalSets === 0) return 0;
      if (a.totalSets === 0) return 1;
      if (b.totalSets === 0) return -1;
      return (b.setWinRate || 0) - (a.setWinRate || 0);
    });

    // Best + Worst vs Middle two
    const team1 = [sortedPlayers[0], sortedPlayers[3]]; // Best + Worst
    const team2 = [sortedPlayers[1], sortedPlayers[2]]; // Middle two

    // Clear the first set and show a message
    setSets([
      { setNumber: 1, scores: Object.fromEntries(players.map((p) => [p.id, null])) },
    ]);
    ...
    setError(null);
    alert(
      `Smart Balance Applied! 🎯\n\n` +
      `Team 1: ${team1[0].name} + ${team1[1].name}\n` +
      `Team 2: ${team2[0].name} + ${team2[1].name}\n\n` +
      `Now enter the scores with teammates having matching scores.`
    );
  };
